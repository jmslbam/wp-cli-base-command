...
trait Bulk_Task {
    ...
    public function loop_posts( array $query_args = [], $callback = false, array $callback_args = [] ) {
        ...
        do {
            ...
            // Otherwise these will be appended to the query
            ...
            // Get them all, you probably have a pretty good reason to be using these.
             ...
             *
             * But it is helpful if you just want to set 1 post ;)
             */
            if ( isset( $query_args['p'] ) || isset( $query_args['post__in'] ) || isset( $query_args['post__not_in'] ) ) {
                \WP_CLI::log( \WP_CLI::colorize('%8Using p, post__in or post__not_in results querying ALL posts, which are not batches by "posts_per_page" and therefor not taking advantage of the goal of this loop.%n') );
                $count = 0;
            }
